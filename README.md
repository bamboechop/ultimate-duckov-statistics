...
The active `feat/economy` branch is the unpublished v0.9.0 candidate. The current immutable implementation is `8aaaacd9533182feb5ffaa8ec3d2889773420f41`: 630/630 Debug and Release tests pass together with the installed Duckov 2.3.30 / Steam 24013657 contract probe, frame-time analyzer, formatting/source-safety checks, warning-free Core/native builds, and exact-five-file package validation. It preserves genuine lifetime degradation without calling that historical limitation a current adapter failure: compact lifetime and run projections now compare scope availability with the separate live adapter snapshot, retaining `current capture unavailable` only for a currently disabled adapter and using `capture unavailable for part of this scope` after a successful restart. Its independently extracted 257,750-byte ZIP is SHA-256 `aeefc0366b3a730a8b0a7a695c896335ac975705726ed62007d74ad7a712af7e`. After exact-hash approval it deployed transactionally with byte-identical readback and no residue. Cold Overview and Runs projections, JSON/24-CSV export, and clean shutdown agree on Money `+0/-1`, Cash `+58/-34`, two completed runs, sequence zero, Supported/Experimental live capabilities, and honestly Unsupported Cash terminal outcomes. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) is open and unmerged at green CI head `d4b4dc75a3ab4d9d2859511d821bfdf64d128474`.
