...
The active `feat/economy` branch is the unpublished v0.9.0 candidate. The current immutable implementation is `d5a3aba701d0d04aa9dd30a7ffa7601bc8f5ef8e`: 624/624 Debug and Release tests pass together with the installed Duckov 2.3.30 / Steam 24013657 contract probe, frame-time analyzer, formatting/source-safety checks, warning-free Core/native builds, and exact-five-file package validation. In addition to the earlier route, arithmetic, recovery-row, saturation, and unavailable-outcome corrections, it rejects cross-scope economy corruption before backup selection, separates completed Cash costs from overlapping player drops, gives a proven physical-Cash sale first claim over an unrelated same-amount Money delta, and durably stores each sequence-zero activation before returning from registration. Its independently extracted 256,866-byte ZIP is SHA-256 `a9ea1f9111c7cca785ea7fbd569a44eccd6f00b7abb82e8f86a024881c26389e`. After exact-hash approval it deployed transactionally with byte-identical package readback, unchanged save/UDS-data trees, and no deployment residue. Cold activation preserved sequence zero and unchanged totals; JSON/24-CSV export and normal shutdown passed with byte-identical profiles, clean diagnostics, and no runtime residue. The prior `7fb82e677ded97c127f900d1e2fde77af03fa66a3e6fb93ad99dfa38d83f19fe` deployment remains historical accepted evidence only. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) remains open and unmerged.
