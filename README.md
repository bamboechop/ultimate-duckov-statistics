...
The active `feat/economy` branch is the unpublished v0.9.0 candidate. The current local implementation is `af3be5ad5946b05cc9227d979a2243e41445dbd2`: 632/632 Debug and Release tests pass together with the installed Duckov 2.3.30 / Steam 24013657 contract probe, frame-time analyzer, whole-solution formatting/source-safety checks, exact package-directory validation, and warning-free Core/native builds. Terminal handling now drains queued Money/Cash and durably checkpoints the refreshed active run before completion clears it; simulated crash recovery proves the terminal flow survives in lifetime, run, segment, completed-run, starting-map, and route-map economy. Compact and detailed Economy projections continue to distinguish persisted scope degradation from current adapter availability. No release ZIP, deployment, gameplay, push, CI, or publication evidence exists for this local correction yet. The installed predecessor remains `8aaaacd9533182feb5ffaa8ec3d2889773420f41`; its approved ZIP, cold compact Overview/Runs validation, JSON/24-CSV export, and clean shutdown remain valid historical evidence but are superseded as final-candidate evidence. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) remains open and unmerged at pushed head `fb2cd7cbd4f3217ca3d798b73cea84eeff60e848`; both `core` and `source-safety` CI checks passed for that pushed head. The local `af3be5a` correction is not yet on the PR.
