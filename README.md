...
The active `feat/economy` branch is the unpublished v0.9.0 candidate. Terminal handling drains queued Money/Cash, captures final movement and integrity, and requires a durable refreshed active-run checkpoint before completion can clear the tracker. That checkpoint retains the intended terminal outcome for crash recovery; failed checkpoint or completed-profile persistence remains retryable without downgrading extraction or death to interruption. Compact and detailed Economy projections distinguish persisted scope degradation from current adapter availability. [PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10), rather than this README, is authoritative for the live remote head, review state, and CI results. Immutable implementation, validation, package, deployment, gameplay, export, and shutdown evidence is recorded chronologically in [TESTING.md](TESTING.md).
