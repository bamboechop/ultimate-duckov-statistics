...
Ultimate Duckov Statistics (UDS) is a local, single-player statistics mod for Escape From Duckov. Version 0.9.0 is the M9 Economy candidate: it retains the complete M1-M8.1 contract and adds exact, separately capability-gated Money and physical-Cash flows.
...
M0-M8 are published through the [v0.8.0 GitHub pre-release](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v0.8.0). M8.1 product commit `90384352d323e6ea19dfa607c7da18162dbcefcb` completed its 507-test, native, package, performance, gameplay, export, and shutdown gates; PR #9 subsequently merged into `main`. Live verification found no `v0.8.1` tag or GitHub release, so v0.8.0 remains the newest published release and the v0.8.1 candidate must not be described as published.

The active `feat/economy` branch is the v0.9.0 candidate. Its latest automated gate passes 589/589 Release tests, the installed Duckov 2.3.30 / Steam 24013657 native contract probe, warning-free Core/native builds, and exact-five-file package validation. The first live economy matrix proved exact Money and Cash amount/direction, failed-purchase exclusion, sale attribution, stack split/merge neutrality, drop/re-pickup flow behavior, export agreement, and clean shutdown, while exposing a false post-extraction Base inflow and a Diagnostics full-tab scrolling defect. Diagnostics scrolling passed the focused retest, but that retest rejected the alias-only Cash correction: scene hydration recorded the carried 37 Cash on raid entry and the returned 44 Cash on extraction as false Base inflows. The corrected adapter suspends Cash reconciliation from level-initialization start/economy load through `OnAfterLevelInitialized`, then establishes one non-economic loaded-inventory baseline. Its focused live retest passed: carried Cash 44 created no flow, corpse-looted Cash 24 remained the sole exact Raid inflow across every projection, and normal shutdown left no persistence residue. A subsequent completion audit corrected base/shop economy persistence so it also uses the M8.1 coalesced snapshot writer instead of synchronously replacing the profile per transaction; the production-linked test proves the on-disk profile stays byte-identical until the deferred snapshot is saved. The focused live smoke passed: Money 178 to 177 produced exactly Base `-1`, Cash stayed 68, export and shutdown persisted the event, and no residue remained. The final source audit additionally made aggregate overflow retain prior exact values instead of clamping and added three regressions. Immutable implementation commit `41721ec47944393d10d1ecae279dea1224f8e4fc` produces the exact 252,244-byte ZIP at SHA-256 `84e2d286e6e2d7f5816d1137ed26cd2bba1fd57c90ccd5ba2bf24e31a877355f`; independent extraction, transactional deployment, installed-byte readback, cold activation, and residue-free shutdown all pass. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) is open and unmerged; its live final-head CI status is reported on the PR.
...
M9 treats account-style `Money` and physical Cash item type `451` as different currencies. Public `EconomyManager.OnMoneyChanged` proves exact post-mutation Money amount and direction without treating load initialization as activity. Completed StockShop sales and `QuestReward_Money` claims can enrich a matching exact delta; all other proven wallet changes remain visible as `UnknownAdjustment` rather than being guessed from sign, price, or UI state. Physical Cash uses event-coalesced, runtime-identity-deduplicated owned totals across storage, main inventory, and pet inventory. Full-scene inventory hydration is suspended until level initialization completes and then becomes a non-economic baseline; carried-in/load-time Cash and stack split/merge/internal movement do not become flows. Successful exact-main-character world-pickup callbacks can become raid acquisition only with a matching positive owned-total delta and bounded drop/re-pickup exclusion. The common corpse/container loot transfer does not emit that callback in the tested build, so its exact Cash delta remains `UnknownAdjustment` and is not fabricated as acquired. The candidate intentionally reports terminal Cash disposition as unavailable: the installed public contract cannot prove secured versus lost units after fungible inventory mixing, so any proven acquired amounts become `Unresolved` at extraction, death, or interruption instead of fabricated secured/lost values.

Economy idempotency uses constant-size activation/sequence replay cursors instead of retaining one identity per transaction. The verified native adapter creates flows synchronously under one random activation ID with strictly increasing sequence numbers; each directly recording aggregate stores only that activation and its closed-through sequence. Lifetime, run, segment, starting-map, and route-map totals therefore continue exactly beyond 2,048 flows without a transaction-count hard stop or an ever-growing ID list. Schema-9 `RecentEventIds` and `DeduplicationSaturated` remain legacy-candidate fields only: they are validated through recovery and compacted after no surviving checkpoint can replay them. An old saturated candidate keeps its exact recorded totals and is marked `LegacyIdentitySaturationIncomplete`; current capture resumes under a fresh activation. JSON exposes the replay cursor and legacy-incomplete marker, while economy CSVs expose `legacy_identity_saturation_incomplete`.
...
```powershell
.\scripts\create-release.ps1 -DuckovPath $env:DUCKOV_PATH -Version 0.9.0
```
