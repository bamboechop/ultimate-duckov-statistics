...
- Current unreleased builds include Runs → Map & Kills: automatic per-enemy encounters, animated walking paths, dotted teleports, separate visits and linked map/feed selection. Weapon/ammunition, damage and inspected loot retain recorded evidence. See [production integration and qualification](docs/COMBAT_ENCOUNTER_PRODUCTION.md).
...
Current source stores changed records in one SQLite database and loads completed-run details on demand. Automatic recovery-copy writes and promotion have been removed; SQLite WAL transactions retain FULL synchronization. Database corruption can make statistics unavailable because there is no automatic duplicate to restore. See [the single-database policy](docs/SINGLE_DATABASE_STORAGE.md). Valid current-format JSON profiles are imported once; SQLite then becomes authoritative. Diagnostics exports `statistics.zip` containing one `statistics.json` file; see the [JSON export contract](docs/JSON_EXPORT.md). Diagnostics can also [restore a saved JSON or ZIP export](docs/JSON_RESTORE.md) after validation and confirmation, replacing only the selected slot's UDS statistics. Current-format validation, interrupted-run/session recovery, generation isolation, deferred replay watermarks and reset/export safety remain required. Older JSON-only builds cannot read subsequent SQLite updates. A failed durability boundary retains pending data with bounded retry and diagnostics. Reset archives only the UDS generation and defaults to Cancel. Export captures an immutable generation snapshot as local zipped JSON.
...
This runs Debug/Release tests, the installed-native probe, warning-free native build, 13-file packaging and artifact audits. The package includes three UDS DLLs, pinned SQLite dependencies and notices, `INSTALL.md`, `LICENSE`, and the tracked `mod/info.ini` and `mod/preview.png`. The exact inventory is maintained in `scripts/package-inventory.ps1`; [SQLite dependency details](SQLITE_DEPENDENCIES.md) identify the bundled libraries. Deployment copies and verifies the full package. The isolated shell suite source-links real panel/shell code and tests access, overflow, input/focus, listener and private-material ownership through boundary doubles. It cannot establish native rendering or GPU resource behavior.

`verify-reproducibility.ps1` builds an immutable commit under two checkout roots and compares all three UDS DLLs, their portable PDBs and deterministic ZIP bytes. `audit-artifacts.ps1 -OrdinaryRelease` checks builder-path leakage and absence of diagnostic IL call sites. `deploy.ps1` requires a closed game, retains a verified previous UDS package outside the native loader scan root and reads back exact deployed hashes.
