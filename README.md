...
M0-M8 are published through the [v0.8.0 GitHub pre-release](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v0.8.0), published on 2026-08-13 after its single-map, multi-map extraction, later-map death, repeated-map re-entry, cross-map activity, abrupt recovery, UI/export, shutdown, and review-hardening gates passed. Controlled M8.1 captures prove that Harmony alone is indistinguishable from the clean game while v0.8.0 adds a CPU-side idle and firing p99 regression. The v0.8.1 candidate replaces per-association equipment rebuilds with a segment-aware event-time cache, checks patch-owner integrity through Harmony shared-state identity stamps, reconciles projectile context once per frame without caching across a run boundary, and moves durable checkpoint and lifetime-profile storage off the game thread without weakening the one-second dirty-state bound, exact interrupted-run recovery, or lifecycle flushes. Its corrected package completes the frozen idle, MP7, MF, and Mosin D/B matrix within the 10% median/20% p99 release ceiling with no repeatable severe cluster. A supplementary three-map run exposed a separate synchronous consumable-persistence hitch; candidate 7 removes the matched completion-aligned hitch, records every Med-Kit activation/heal exactly once, and passes the same hard ceiling. Candidate 8 adds cross-segment equipment-cache correctness and passes 505/505 tests plus exact transactional deployment. Its first natural-play soak remained smooth through Nullpunkt and Lagerbereich, with exact UI/JSON/CSV projection and clean shutdown. The retry reached Nullpunkt → Lagerbereich → Farmstadt, retained complete equipment roots and 891 unique associations, persisted all three Med-Kits plus food and hydration exactly once, showed only 1.1% late-versus-early p99 movement, and produced no perceived UDS-related hitch. Its UI, exported JSON and nineteen CSVs, live profile, backup, and Info-only diagnostics agree exactly. Normal shutdown consumes the pending-save marker, removes all session/checkpoint/temp residue, and leaves byte-identical primary/backup state. The matched hard-ceiling evidence therefore classifies v0.8.1 as acceptable. Immutable product commit `c2276aa4fc6629c4e71bb0302401ff1abc267792` produces the exact five-file 221,742-byte ZIP at SHA-256 `853f26cec49048e68554e6a2e7a3804b6aa1b9dfbbe5a91072821bcb5433563a`; independent extraction, transactional deployment, cold-start activation, capability readback, and normal shutdown all pass. [Draft PR #9](https://github.com/bamboechop/ultimate-duckov-statistics/pull/9) is open, draft, and unmerged for independent review. M9 economy and M10 full UI/release hardening remain future work. UDS has not been uploaded to Steam Workshop.
