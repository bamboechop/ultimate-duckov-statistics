...
- Typed schema-9 `CurrencyFlowRecorded` events with stable event/save/run/segment/map identity, positive amount, explicit direction, currency, source, context, integrity, adapter provenance, producer activation, and monotonic producer sequence. Directly recording aggregates use one constant-size closed-through replay cursor, so exact Money/Cash capture has no fixed event-count stop. Completed-run aggregation is guarded by run identity and exact totals rather than transaction IDs.
...
- Schema-8 migration that preserves all prior statistics and marks pre-M9 economy unavailable, current-schema root validation and backup selection, exact-once deferred lifetime recovery, stale-checkpoint protection, saturating counters, composition repair, and independent profile/run publication.
...
The scalable-idempotency correction passes the complete 605-test Release suite, including a 100,000-event stress, constant replay metadata, stable serialized profile size, exact duplicate/recovery windows, schema-9 legacy migration, malformed-candidate selection, multiple activations, save rotation, and economy aggregation beyond 2,048. The installed-game contract probe, frame-time analyzer, warning-free Core/native Release builds, formatting, source safety, and exact five-file package-directory validation also pass. Immutable implementation commit `ea49808897c018bfbe437859475acb2c6240fb37` produces the independently extracted and verified 253,717-byte `UltimateDuckovStatistics-v0.9.0.zip` at SHA-256 `2ef0899013319b91bd8771b18e805fa780c82300726f7ba34a495d4a90b4b1b7`; both DLLs report product version `0.9.0+ea49808897c018bfbe437859475acb2c6240fb37`. Approved deployment/readback, the minimal cold persistence check, and updated draft-PR CI are still pending. The paragraph below records the now-superseded 2,048-hard-stop candidate and is retained only as historical evidence; none of its commit, ZIP, deployed hashes, or cold-start claims identify the corrected source.

The latest Release suite passes 589/589. The installed Duckov 2.3.30 / Steam 24013657 / Unity 2022.3.62f2 / Harmony 2.4.1 contract probe includes the M9 public economy hooks and the level-initialization boundary. Core and native Release builds complete with zero warnings and errors, and the package root validates as exactly five permitted files. The first live matrix proved zero-flow load, unaffordable-purchase exclusion, a Money `Sale +179`, an unknown Money purchase `-1`, corpse-looted Cash `+37`, split/merge neutrality, drop/re-pickup `-37/+37`, JSON plus twenty-three CSVs, and residue-free clean shutdown. It also exposed a Diagnostics full-tab scrolling defect and a false extraction-time Base inflow. Full-tab scrolling passed the second retest, but the lifetime result `Base +81, Raid +7` proved the remaining defect was scene hydration: carried Cash 37 was observed while entering the raid, then returned Cash 44 was observed after extraction. The run itself remained exactly Raid `+7`. Adapter contract `native-economy/2.3.30+public-events-v2` suppresses owned-Cash scans during full-scene hydration and establishes one baseline after `OnAfterLevelInitialized`. Its focused live retest passed: carried Cash 44 produced no flow, newly corpse-looted Cash 24 was the only exact Raid inflow in lifetime and every run/map projection, and clean shutdown left byte-identical profiles with no pending state. A subsequent completion audit found that base/shop flows still bypassed the M8.1 deferred writer and synchronously replaced the profile once per transaction. The latest source corrects that boundary and proves a base Money mutation leaves the persisted profile byte-identical until the coalesced snapshot writer runs, while active-run watermark recovery remains exact once. The focused live smoke then proved ATM Money `178 → 177` for one Geheime Nachricht, unchanged physical Cash 68, exact UDS Base outflow 1, JSON/twenty-three-CSV agreement, deferred durability through clean shutdown, and no pending or deployment residue. The final source audit also corrected aggregate-to-aggregate overflow so the prior exact currency and Cash-outcome values are retained instead of clamped, while an independently representable currency still merges; three regressions cover those recovery boundaries. Immutable implementation commit `41721ec47944393d10d1ecae279dea1224f8e4fc` produces `UltimateDuckovStatistics-v0.9.0.zip` at 252,244 bytes and SHA-256 `84e2d286e6e2d7f5816d1137ed26cd2bba1fd57c90ccd5ba2bf24e31a877355f`; the sidecar, independent extraction, exact five-file deployment/readback, cold activation, and residue-free clean shutdown pass. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) is open and unmerged; its live final-head CI status is reported on the PR. Progressed-save recovery was not required. No v0.9.0 tag, release, merge, PR-ready transition, or Workshop upload has occurred.
