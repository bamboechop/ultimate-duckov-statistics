...
M9 adds exact, separately capability-gated Money and physical-Cash statistics without changing the accepted M1-M8.1 semantics. This remains an unpublished draft candidate. Earlier identity, sequence-zero, and `20fa8ac` deployments are superseded as final candidates. The current `c4f020e` replacement has passed the complete automated pipeline, fresh exact-hash deployment/readback, cold Economy UI check, and clean shutdown; historical live merge/export evidence remains documented below.
