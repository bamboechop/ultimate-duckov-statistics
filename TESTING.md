...
10. Installed-game contract probe covers character-slot/hold/ordinary-inventory events, `CharacterItem`, `CurrentHoldItemAgent`, item-tree events, slot content, tags, ordinary inventory, and attached Tote inventory.
...
- The first transactional deployment of the superseded `6a851f2` package was valid for its then-current artifact but is no longer the candidate deployment. After the Tote-root correction, transactional deployment again replaced only `E:\SteamLibrary\steamapps\common\Escape from Duckov\Duckov_Data\Mods\UltimateDuckovStatistics`. Package, staging, and promoted-directory validation passed; independent readback found exactly the five current candidate files and matched every SHA-256 recorded above. Both deployed DLLs report file version `0.6.0.0` and product version `0.6.0+1517f0294d93587655d2a15c173947bc79697a56`. No deployment staging or previous-directory residue remains, and Duckov stayed closed throughout.
...
The slot-1 technical manual matrix remains accepted, but completion is reopened pending the required slot-2 Tote presence, export, restart, and clean-shutdown follow-up on corrective implementation `1517f02`. No merge, tag, GitHub release, ready-for-review transition, or Workshop upload is authorized by this protocol.
