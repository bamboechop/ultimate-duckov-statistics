...
| --- | --- | --- |
| Automated Release suite | Passed after gameplay repair 2026-08-09 | 88 tests; full M0/M1 regression plus immediate/delayed healing, full/partial overheal, interleaving, provenance, restart, migration/recovery, delayed-healing group repair, capability, export, and forbidden-package cases |
...
| Five-file package audit | Passed and repaired candidate deployed 2026-08-09 | Exact inventory; no forbidden DLL; deployed Core SHA-256 `2499151e7ff4fb197e65c141846100d0470789892b7a4e971e5a5cf0334fa3b8`, adapter SHA-256 `7743d50b779b225797347dd43e5726c265f98548cee5b95da7db3089a5af709b`; every file matched and no deployment residue remained |
...
| Source, draft PR, CI, ZIP, and checksum | Passed 2026-08-09 | `feat/healing-attribution` is pushed; draft unmerged PR #2 targets `main`; required `source-safety` and `core` checks are green on the current head; independently verified 62,435-byte ZIP and matching lowercase SHA-256 sidecar are ready |
...
Final v0.2.0 artifact evidence, 2026-08-09: `create-release.ps1` reran all 88 Release tests, the Duckov/Harmony contract probe, the warning-free native build, and five-file package validation. `UltimateDuckovStatistics-v0.2.0.zip` is 62,435 bytes with SHA-256 `700ae5372060b6d191a579b633d80cd96e4bf678257b951ac9765c9fd4102e28`; the lowercase sidecar matches an independent recomputation. Independent extraction contains exactly one `UltimateDuckovStatistics` folder with `info.ini`, `INSTALL.md`, `LICENSE`, `UltimateDuckovStatistics.Core.dll`, and `UltimateDuckovStatistics.dll`. The extracted folder passes `verify-package.ps1`, contains no forbidden Duckov, Unity, framework, or Harmony DLL, and every file is byte-identical to the audited source package.

Delivery evidence, 2026-08-09: the complete M2 source is pushed on `feat/healing-attribution`. Draft PR [#2](https://github.com/bamboechop/ultimate-duckov-statistics/pull/2) is open against `main`, remains draft and unmerged, and has green required `source-safety` and `core` checks on the current branch head. The GitHub Actions Node 20 deprecation annotations concern upstream `actions/checkout@v4` and `actions/setup-dotnet@v4`; GitHub forced those actions onto Node 24 and both jobs completed successfully.
