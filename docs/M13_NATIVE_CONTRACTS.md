...
UDS patches the private `Craft(CraftingFormula)` invocation and the exact internal `Cost.Return(bool, bool, int, List<Item>)` task it calls. A private-craft prefix captures formula evidence and opens a thread-scoped correlation before native code invokes `Cost.Return`; the return postfix wraps only `Return(false, true, 1, generatedBuffer)` from that exact scope. Successful return-task completion proves the full declared output delivery and publishes one action before native code proceeds to downstream callbacks. The private craft wrapper still observes rejection, synchronous failure, and contract drift, but a later callback exception cannot retract already-proven delivery. The completion timestamp is read at the correlated delivery boundary. Ordinarily the then-current UDS generation is assigned there; if Duckov has already selected another save or a UDS profile reset has started while that profile transition remains queued, the mutation is instead staged under the exact transition ID and receives only the committed target generation. A null result or exception before successful delivery abandons the in-flight token and publishes nothing.
...
| Multiple-output recipes | Omitted baseline limitation | Installed `CraftingFormula` has one `ItemEntry result`, not an output collection |
| Workstation identity | Omitted baseline limitation | `InteractCrafter` opens the shared view with a tag predicate but does not propagate a stable workstation identity into `Craft` |
| Run/map/context attribution | Omitted baseline limitation | The completion call exposes no reliable run, map, or crafter context; M13 remains save-generation lifetime scope |
| Ingredients and total charge | Independent M16 payment proof | See [M16 native contracts](M16_NATIVE_CONTRACTS.md); output delivery alone cannot prove costs. Value/profit are omitted. |
...
- profile transitions flush prior pending aggregates before rotation; after a native save selection or UDS profile reset starts, later proven completions are staged by the newest queued transition ID and transferred only after that transition commits its target generation. FIFO transition completion preserves overlapping selections, and export or cleanup cannot discard an uncommitted handoff.
