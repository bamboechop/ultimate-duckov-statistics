...
### Item Use historical run healing

Recent-run healing captions and per-item healing values now also require the persisted run completeness flag and intact history/attribution evidence. Current supported capture cannot remove a prior run's Unavailable or Partial qualifier. Item-use counts and amounts remain independently available. The existing production-path regressions now reopen the profile, remove foreign Harmony patches, activate and publish supported healing capture, then compare Runs with Item Use captions and item rows for unavailable zero, partial positive evidence and fully supported capture. All 1,877 Release tests, contract probe, zero-warning native build and package checks passed; deployed with Duckov closed and all five installed files matched the verified package.

### Lifetime healing capture gaps

Profile statistics now persist a lifetime healing completeness flag independently of retained run rows. Failed native healing capability publication clears it in memory before persistence; item use while capture is unavailable and completion/recovery of an incomplete run also retain the gap. Deferred profile snapshots copy the flag. Supported capture never sets an existing generation back to complete. New generations begin complete; deserialized profiles missing the scalar remain loadable and conservatively incomplete. Overview healing, Item Use lifetime healing and lifetime item rows require both this evidence and current support. Counts and recorded healing amounts are unchanged.

The activation-loss/restart regressions now check lifetime totals and item rows, including positive observations followed by unobserved native-shaped healing, locked persistence, supported publication after restart, and a separately reloaded snapshot with run history removed. They also check missing lifetime completeness metadata without rejecting the profile. History removal is a test simulation, not a claim of a shipped history-pruning operation. All 1,877 Release tests, installed-game contract probe, zero-warning native build and package validation passed. Deployed with Duckov closed; all five installed files matched the package.

### Lifetime healing export evidence

The flattened statistics.json document now carries HealingCaptureComplete, HealingCaptureState and HealingEvidenceRepaired at lifetime scope. overview.csv, groups.csv and items.csv retain numeric actual_hp_restored unchanged and place healing_capture_complete, healing_capture_state, healing_evidence_repaired and healing_evidence_state beside it. The row evidence state is Supported only for complete, supported, unrepaired capture; otherwise positive observations are Partial and zero is Unavailable. CSV consumers should resolve columns by header name.

The production activation-loss/restart regressions now call NativeProfileCoordinator.ExportCurrent and inspect the generated JSON and all three CSV files. They cover unavailable zero, positive partial observations, exact zero/value, restored current support and the temporary storage-failure route. All 1,877 Release tests, installed Duckov contract probe, zero-warning native build and package validation passed. Deployed with Duckov closed; all five installed files matched the verified package. No user profile export was performed for verification.
