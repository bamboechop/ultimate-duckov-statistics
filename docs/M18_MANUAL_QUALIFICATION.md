...
The user selected slot 3 for native reset/recovery checks. [Reset evidence](M18_SLOT3_RESET_QUALIFICATION.json) verifies one new generation, an archive preserving both prior runs, zero recorded actions in the fresh profile, retained current holdings and unchanged slot 1 statistics/settings. Cancel retaining the two runs and the confirmed reset showing an empty Runs page await explicit UI confirmation. The reset procedure remains available for regression checks: first cancel and verify both runs remain, then confirm once and export. Slot 1 remains intact for later history/performance qualification. A copied current backup also passed isolated production-repository recovery with no generation rotation; native recovery and a cold reopen of the reset generation remain open.
