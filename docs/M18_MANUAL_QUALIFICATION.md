...
The user selected slot 3 for the upcoming native reset/recovery checks. Its [preserved baseline](M18_SLOT3_RESET_QUALIFICATION.json) has two runs. First cancel the reset prompt and verify both remain; then confirm one reset, verify an empty Runs view and export. Slot 1 remains intact for later history/performance qualification. This sequence is prepared and has not yet been exercised.
