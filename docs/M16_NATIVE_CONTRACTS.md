...
M16 adds independent capabilities for item-resource identity, output/resource association, total currency charge, and Money/Cash split. Invalid, unreadable, or insufficient repeated-entry payment evidence disables the first two without disabling M13 completion/output totals or exact currency. Invalid currency evidence disables only total currency. A later exact event may still be retained as partial history, but the degraded capability never silently returns to complete. A foreign patch or runtime patch drift on `GetItemCount`, `StackCount`, or `MarkDestroyed` disables only item-resource capture as described above. Patch drift across the shared craft, delivery, or payment methods, version mismatch, or an invalid core delivery contract retains M13's adapter-wide fail-closed behavior.
