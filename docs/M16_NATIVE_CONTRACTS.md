...
M16 adds independent capabilities for item-resource identity, output/resource association, total currency charge, and Money/Cash split. For an individual craft, invalid or unreadable invocation evidence and insufficient repeated-entry payment proof are provisional until delivery: if the craft is rejected, fails, remains incomplete, or is abandoned, they change no capability or history. Only a successfully delivered mutation with unproven resource evidence atomically disables item-resource identity/output-resource association and marks resource history incomplete while retaining M13 completion/output totals and exact currency; a delivered mutation with invalid currency evidence applies the equivalent restriction only to total currency. The production reducer performs that event, capability, and partial-history transition in one accepted aggregate mutation. The adapter immediately stops treating the affected evidence as supported in memory, stages the matching coordinator snapshot, and publishes it on the deferred adapter tick; the native request, payment, and delivery callbacks perform no full capability/profile save. A later exact event may still be retained as partial history, but a committed degraded capability never silently returns to complete. A foreign patch or runtime patch drift on `GetItemCount`, `StackCount`, or `MarkDestroyed` is independently authoritative runtime contract loss and immediately disables only item-resource capture as described above. Patch drift across the shared craft, delivery, or payment methods, version mismatch, or an invalid core delivery contract retains M13's adapter-wide fail-closed behavior.
