...
All aggregates use checked arithmetic. On overflow, UDS preserves the prior exact value and disables only the affected resource-action, resource-quantity, currency-action, or currency-amount projection. While both currency dimensions remain exact, actions and amount must either both be zero or both be positive at the lifetime, output, and recipe levels. When only currency actions are unavailable, a pair with zero actions and positive exact amount is legitimate because action updates stopped while amount updates continued; positive actions with zero exact amount is impossible. When only currency amount is unavailable, positive actions with zero amount is legitimate while zero exact actions with positive amount is impossible. If both dimensions are unavailable, either frozen shape may remain. Current-schema candidate validation applies that directional state machine before atomic selection so a corrupt primary cannot defeat an intact backup.

Resource quantity and currency are reducer-atomic fan-outs: one proven resource event updates its lifetime total and exact output/recipe/resource association together, while one proven currency event updates lifetime, output, and recipe totals together. Unproven evidence updates none of those levels, and the checked preflight skips the whole affected dimension before any overflow. `ApplyResourceQuantity` creates a lifetime resource row only while applying a positive canonical quantity; a first-event saturation creates no lifetime row, and later saturation preserves the prior positive value. A persisted lifetime resource row is therefore always strictly positive regardless of `ResourceQuantityArithmeticUnavailable`. Each exact resource mutation also contributes at least one quantity per consumption action, so `ConsumptionActions <= ConsumedQuantity` while quantity arithmetic is exact. Quantity overflow stops later quantity updates without stopping independently exact action updates, so that pair constraint intentionally relaxes only when `ResourceQuantityArithmeticUnavailable` is set. Capability or history degradation changes the interpretation of retained history but cannot create a legitimate subset between fan-outs. Current-schema validation always requires equality across them; this does not change the separately supported recipe-identity and batch-metadata subset semantics. Save-generation handoff, deferred publication, retry, atomic primary/backup/temporary recovery, and terminal-shutdown ownership remain the M13 production boundaries.
