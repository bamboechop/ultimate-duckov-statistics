...
On save 3, startup Diagnostics marked Menu access Limited solely because the base pause-menu entry was `NotObserved`. The presentation now reserves Limited for an observed unavailable entry. Unobserved and attached-but-unexercised rows retain their precise neutral status; the group remains Working, and the banner does not claim every entry was verified. The correction passed 50 focused Release Diagnostics tests, changed-file formatting/analyzers and a zero-warning installed-native Release build. Replacement package/deployment and native readback of this correction remain Not exercised; the artifact identities above still describe the previously deployed candidate. Current correction delivery is tracked in [PR #19](https://github.com/bamboechop/ultimate-duckov-statistics/pull/19).

The same save's three Experimental Economy capabilities match the supported native contract: exact Money changes have only partial semantic source/context attribution, and proven external Cash acquisition does not cover corpse/container transfers or ambiguous player-originated split identities. Their truthful Limited status is retained; this is partial supported capture, not a permanently impossible adapter or a newly observed tracking failure.
