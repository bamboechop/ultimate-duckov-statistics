...
## Dedicated-column comparison — 2026-09-13

Following the [disposable column-storage comparison](SQLITE_COLUMN_COMPARISON.md), the user decided on 2026-09-13 to retain this implementation and drop the blanket conversion to dedicated columns. Fully converting remaining payloads to typed columns with shared string IDs increased compacted primary size by 10.18% on the recorded three-run profile and 18.60% on the existing 1,000-run fixture. A hybrid retaining compressed run history saved only 4 KiB on the recorded profile and was slightly larger on the large fixture. All recorded values and export streams were preserved. Prototype reconstruction timings include a JSON compatibility bridge; they do not establish the performance of a future direct typed reader. No production format change followed from this comparison.
