...
M12 accumulates high-frequency clock callbacks in memory and publishes one checked aggregate mutation at a bounded one-second cadence through the existing deferred profile snapshot writer. It flushes before export, save-generation transitions, application quit, deactivation, and clean disposal. Profile changes clear any uncompleted sleep candidate; the next clock observation establishes a new baseline without assigning prior observations to the replacement generation. M12 never creates run, route, map, or segment attribution because the sleep UI is a base interaction and no additional useful raid identity is exposed by this contract.
