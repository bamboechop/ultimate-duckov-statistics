...
`GameClock.Load` hydrates `days`, `secondsOfDay`, and real-time-played state from the active Duckov save and invokes the same `OnGameClockStep` callback. UDS therefore establishes or replaces a generation-local baseline without counting the first observation. It never treats the callback alone as proof of hydration: `SavesSystem.SetFile` leaves the prior clock running, so a slot/deletion handoff captures that prior `GameClock.Instance` and ignores every callback from it until a replacement instance reports. If the replacement load arrives while the UDS profile transition is still retrying, its baseline and later forward observations remain staged until that exact transition ID commits. Installed `LevelManager.Start` invokes `OnNewGameReport` and immediately calls `OnNewBoot`; UDS snapshots the already-loaded coordinate at report entry, buffers the ensuing 07:00 advancement, and transfers that delta only after the matching new-generation transition commits.
