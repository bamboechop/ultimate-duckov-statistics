...
`GameClock.Load` hydrates `days`, `secondsOfDay`, and real-time-played state from the active Duckov save and invokes the same `OnGameClockStep` callback. UDS therefore establishes or replaces a generation-local baseline without counting the first observation. It never treats the callback alone as elapsed time. Installed `LevelManager.Start` then invokes `OnNewGameReport` and immediately calls `OnNewBoot`. Because the clock is already loaded at the report boundary, a new-game UDS rotation resets directly to that current native coordinate before control returns to `OnNewBoot`; the ensuing 07:00 advancement is therefore counted in the new generation. Save-slot selection and save deletion use a distinct reset mode that waits for the next native load observation instead of sampling a possibly stale prior-slot clock.
...
`GameClock.Awake` establishes its singleton, subscribes its private save writer to `SavesSystem.OnCollectSaveData`, and performs the native load. `SavesSystem.OnSetFile`, `OnSaveDeleted`, `OnCollectSaveData`, and `LevelManager.OnNewGameReport` remain the established UDS profile-generation boundaries. World-time profile changes explicitly distinguish an already-loaded new-game/report boundary from a slot/deletion boundary awaiting native load, so the first post-report boot step cannot be mistaken for hydration.
