...
## Follow-up: repeated focus feedback during live refresh

Commit `d555ade` removes unconditional control clearing during same-generation refreshes in Economy, Item Use, Crafting, Equipment, and Combat. Clearing disabled the selected button, reset its tint, and caused focus restoration to replay native selection/hover audio. These views now retain their controls until rebind; invalidation still clears immediately when the profile changes or becomes unavailable. Existing row identity binding continues to cancel presses when a pooled control changes identity.
