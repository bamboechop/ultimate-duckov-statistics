...
The panel owns one retained shell with the native header, back control, horizontal tabs and all nine tab views. Closing deactivates and destroys the root and releases its listeners and owned resources. Opening captures cursor and EventSystem focus; closing restores the prior live selection. The source-only legacy IMGUI renderer is not a runtime entry point. [Retained Diagnostics](M17_RETAINED_DIAGNOSTICS.md) owns settings, native health and the shared reset/export controller; its modal blocks obscured controls and gives Cancel initial focus.
...
The UI projection is built only when the active profile generation, statistics generation, and coordinator generation agree exactly. Tab presentations are detached at open and refreshed after profile-change notifications, coordinator generation changes or recorded revision changes; invalidation clears old content before a new generation can be presented. Diagnostics also refreshes when its save receipt, bounded log or native menu evidence changes. A failure to prove that relationship shows the localized unavailable response and never substitutes another profile or an invented zero state.
