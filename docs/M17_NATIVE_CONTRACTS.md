...
The installed `MainMenu` scene supplies the stronger Gate 1c shared-shell treatment. `Canvas/MainMenuContainer/Menu/OptionsPanel/Text (TMP)` is a real major heading: `ResourceHanRoundedCN-Medium SDF`, `ResourceHanRoundedCN-Medium Atlas Material Shadow`, normal style, regular weight, zero character/word/line/paragraph spacing, left alignment, and serialized size `116.75`. UDS rejects the `Canvas/MainTitle` logo text as a panel-heading source. `Canvas/MainMenuContainer/Menu/OptionsPanel/Return` is a native back button whose presentation consists of `Button`, `ProceduralImage`, `RoundModifier`, and the child sprite `pictoicon_arrow_line_prev`; layout, punch, fade, and source-action behaviours are not retained. The rounded tab source is `OptionsPanel/Tabs/Common`, its cyan rail is `OptionsPanel/Tabs/Image`, and the rounded content/frame source is `OptionsPanel/ScrollView/Background`. Those sources use the installed `procedural_ui_image_default_sprite` plus paired procedural modifiers (`OnlyOneEdgeModifier` or `UniformModifier`, with radii 5, 15, or 16 on the audited objects). UDS clones only already-loaded presentation instances while its hierarchy is inactive, preserves each `ProceduralImage`/modifier pair, disables inherited action and layout behaviours before activation, and replaces click events. It neither extracts nor redistributes those installed assets.

The menu entry uses the live native button presentation. Visual-correction Gate 1 replaces the visible immediate-mode panel with one retained Unity UI hierarchy attached to a proven active screen-space Duckov canvas. Gate 1c composes a dimmer, rounded translucent navy frame, distinct rounded content surface, verified major-heading role, native rounded back control, native rounded tabs, light selected text on cyan, distinct inactive/hover/pressed/focused/disabled states, and the native cyan rail. If a source is absent on another supported access surface, the shell uses a restrained code-owned fallback rather than guessing an unloaded asset. It packages no font, sprite, background image, or other visual asset. The hierarchy is created once per opening, retained unchanged while open except for resolution, localization, and selection updates, and destroyed on close.
