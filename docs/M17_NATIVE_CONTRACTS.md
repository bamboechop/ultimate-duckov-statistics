...
UDS subscribes once during panel initialization and unsubscribes on disposal. Main-menu attachment is retried for an already-created menu and on every later menu-awake signal. Pause attachment is permitted only while the installed level manager proves base. Closing or destroying the source menu closes the one shared panel instance. Raid access remains rejected independently at the panel boundary, so a stale or unexpected menu callback cannot expose the panel during gameplay.
