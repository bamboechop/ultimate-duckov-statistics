...
The same audit explains why the complete native tab and surface hierarchies are inspected but not cloned into the shared shell. `OptionsPanel/Tabs/Common` contains an opaque white 40x40 child `Image` using `settings_48dp_FFFFFF_FILL1_wght400_GRAD0_opsz48` and a full-stretch cyan `SelectionIndicator` in addition to its root target graphic. Cloning that complete hierarchy would place the Settings glyph over every UDS label and leave every cloned indicator active, visually overriding the root button's selected/inactive transition. `OptionsPanel/ScrollView/Background` is a native `ProceduralImage` composed for its original scene hierarchy, not a dependable full-screen shell layer after reparenting. Gate 1c therefore constructs each tab as exactly one UDS-owned `Button`, one root `Image` target graphic, and one TextMeshPro label; it copies only safe root presentation properties from the public `GameplayDataSettings.UIPrefabs.Button`. The five-pixel cyan rail remains a plain UDS-owned `Image`. The already-loaded heading typography and sanitized native back remain the only shared-shell clones. UDS neither extracts nor redistributes installed assets.

The menu entry uses the live native button presentation. Visual-correction Gate 1 replaces the visible immediate-mode panel with one retained Unity UI hierarchy attached to a proven active screen-space Duckov canvas. Gate 1c-A separates invisible layout ownership from visible surface ownership: the full-screen root retains the `0.68` input-blocking dimmer; an invisible full-screen `LayoutRoot` owns the frozen title, native rounded back control, clean root-owned tabs, light selected text on cyan, distinct inactive/hover/pressed/focused/disabled states, UDS-owned cyan rail, and transparent lower content infrastructure; and exactly one independent rounded header background is visible. There is no full-height frame graphic and the lower `ContentPlaceholder` `Image` has alpha zero.

The header background is not a panel clone. It is one UDS-created `UnityEngine.UI.ProceduralImage.ProceduralImage` plus one pre-attached `UniformModifier`, constructed while the shell root is inactive. The installed `Plugins.dll` contract proves public writable `BorderWidth`, `FalloffDistance`, and `Radius`; the component starts with border width zero, falloff one, and an 18 px radius at the 2560x1440 baseline. Its exact color is `Color32(0, 8, 16, 140)`, with no descendant, controller, copied material, or alpha-multiplying parent group. The source alpha is about `0.549`; composed over the unchanged dimmer it transmits about `14.43%` of the original scene. The transparent lower layout transmits the dimmer's unchanged `32%`.

At 2560x1440, the header bounds are `84.992,111.992` through `2475.008,329.992`, the CanvasScaler-safe sub-pixel form of the measured `85,112` through `2475,330`. The transparent lower layout is `84.992,369.992` through `2475.008,1410`, leaving a 40 px gap. Responsive surface geometry is derived from the unchanged e874589 child insets and dimensions; narrow mode scales its extra padding, gap, radius, and lower margin with safe minima while retaining the existing title/tab and horizontal-overflow policy. If a source is absent on another supported access surface, the shell uses a restrained code-owned fallback rather than guessing an unloaded asset. It packages no font, sprite, background image, or other visual asset. The hierarchy is created once per opening, retained unchanged while open except for resolution, localization, and selection updates, and destroyed on close.
