...
`GameplayDataSettings.UIPrefabs.Button` and `.ScrollRect` are public native prefab references. `GameplayDataSettings.UIStyle.TemplateTextUGUI` exposes Duckov's installed TextMeshPro font, shared material, style, weight, kerning, padding, and character/word/line/paragraph spacing through a public template, and `CanvasScalerController` proves that the installed screen-space UI responds to resolution changes through its `CanvasScaler`. The exact live Settings/Options/Mods button selected as the menu anchor supplies the title and navigation typography role; the public template supplies body and secondary roles and is the checked fallback for every role when no live anchor text is available. UDS snapshots those already-loaded references once at injection/open, logs their runtime object, font, material, sizing, style, weight, and spacing names, and applies the shared assets without cloning a material or performing per-frame lookup. `GameplayDataSettings.UIStyle.FallbackItemIcon` is the installed generic icon. `GameManager.EventSystem` exposes the active Unity event system. `Duckov.UI.NotificationText.Push(string)` is the installed transient-feedback path.

The menu entry uses the live native button presentation. Visual-correction Gate 1 replaces the visible immediate-mode panel with one retained Unity UI hierarchy attached to a proven active screen-space Duckov canvas. Gate 1b keeps that architecture while making its shared hierarchy explicit: a translucent navy near-full-screen frame, a substantially larger left-aligned title, a code-drawn top-left back control, taller native-font navigation, light selected text on cyan, distinct inactive/hover/pressed/focused/disabled states, a cyan navigation rail, and a bounded translucent content placeholder. The shell copies presentation assets from the public native button reference, copies scroll motion settings from the public native scroll reference, and assigns only already-loaded installed TextMeshPro assets to mod-owned text components. It packages no font, sprite, background image, or other visual asset. The hierarchy is created once per opening, retained unchanged while open except for resolution/state updates, and destroyed on close.
...
The panel has one exact navigation sequence: Overview, Runs, Records, Combat, Equipment, Economy, Crafting, Item Use, Diagnostics. Gate 1 stretches the retained shell within physical-pixel margins of the active native canvas. Control and typography dimensions are normalized by that canvas's scale factor, so the title and tab strip remain readable rather than shrinking to the canvas reference resolution. Each tab uses the native font's measured preferred width plus bounded padding, never destructive auto-sizing; representative long localized labels widen the one-row content up to a defensive per-tab bound and therefore enter horizontal scrolling rather than forcing smaller text. The tabs show leading/trailing directional cues only when more content exists in that direction, and the selected keyboard tab's actual laid-out bounds are scrolled back into view. The retained content host is a separately clipped, clamped vertical `ScrollRect` with the same directional-cue policy; Gate 2 will populate its exposed content root and reconnect the existing multi-column and bounded-history projection policies when view bodies are ported.
