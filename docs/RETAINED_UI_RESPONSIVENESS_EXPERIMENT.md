...
- A profile transition closes and disposes the cache. A different native canvas, an externally destroyed hierarchy, or a failed construction/bind also causes rebuilding. No cached data is reused across generations. Final mod disposal releases all owned UI resources.
...
1. Launch Duckov freshly, reach the main menu and press F9 before opening UDS.
...
6. Load the usual save, start another F9 recording, open/close UDS in base, sell or craft a small amount, then reopen and check updated values. Finish with F10 and close the game.
