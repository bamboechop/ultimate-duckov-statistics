...
Confirmed UDS reset: F8 rotates only the UDS generation and does not alter Duckov Money, Cash, inventories, or saves. A transition-ID-bearing reset handoff records Money trust only when the adapter already received authoritative hydration or `OnMoneyChanged` evidence from the exact live `EconomyManager` instance. If reset completion retains that same instance, the new schema-15 snapshot immediately re-observes its unchanged Money even when the first native `EconomyData` save key does not yet exist. Missing prior trust, a null or different manager, or any pending save-slot handoff cannot use this reset exception and remains behind the normal hydration/replacement gate. Cash is independently re-read from the still-live authoritative roots. Reset must never manufacture zero holdings or carry a different save's Money.

Confirmed restart/recovery: a persisted snapshot is not live native confirmation. On profile open, any valid matching-generation component is downgraded from `Current` to `LastObserved`; an absent, invalid, mismatched-generation component is `Unavailable`. Recovery never derives holdings from M9 inflow/outflow aggregates. Liquid wealth is recomputed only while both components have a live `Current` observation.
