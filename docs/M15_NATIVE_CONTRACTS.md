...
Confirmed save selection: `SavesSystem.SetFile(int)` updates `CurrentSlot` before its synchronous `OnSetFile`. Native subscribers can run in either subscription order. M15 therefore stages load/change notifications and publishes no observation until the UDS `ProfileChanging` / `ProfileChanged` transition has opened the matching generation. Cash additionally waits for the replacement scene roots. Nested backup restoration can invoke more than one `OnSetFile`; the same generation gate applies to every staged observation.
