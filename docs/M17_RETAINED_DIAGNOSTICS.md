...
| Tracking health | A failed supported metric makes its affected system Error and the tracking banner Error. Independently working rows remain usable. The three Economy contracts with expected partial source/context/acquisition coverage are Working when active; their raw capability state, provenance and data-coverage notices remain intact. Unexpected Experimental capability states remain Limited. |
| Native menu fallback | An unavailable menu entry makes Menu access Limited. Unobserved or attached-but-unexercised entries retain neutral details and do not downgrade the group. The configured outside-raids hotkey remains available, and access failure alone leaves the tracking banner healthy. |
