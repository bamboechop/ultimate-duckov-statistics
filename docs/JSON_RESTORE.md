...
Once the replacement directory is promoted, a failed SQLite reopen leaves that committed replacement awaiting activation. The same request retries activation without rotating again or applying the original destination-generation check to the new generation. A different preview, slot or expected generation cannot take over the pending restore. Observer handoff and completion follow successful activation; changes accepted while it waits retain their new-generation journal. The shared user-reset path follows the same rule.
...
Review correction on 2026-09-13: four production-coordinator regression cases inject failures at storage construction and load after promotion, for both restore and reset. They verify pending status, a single unchanged archive and generation, request isolation, exactly-once observer handoff, retained movement accepted during failure, continued run recording and a subsequent native profile reopen. Debug and Release each passed 2,229 main tests and 91 shell tests; the native contract probe, native Release build and 13-file package/IL audit also passed. Evidence is retained in `artifacts/restore-activation-build.log`. These injected failures are automated qualification, not an in-game reproduction.
