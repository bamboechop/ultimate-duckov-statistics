...
10. **M8.1 — Performance investigation and hot-path hardening (`v0.8.1`, delivery candidate complete; draft PR #9)**
    ...
    - The corrected production candidate completes comparable cold-launch, same-map UDS-disabled and UDS-enabled idle, empty-space firing, and single-enemy firing cells with two automatic weapon classes plus a slower cross-class control. Every reproducible cell passes the 10% median/20% p99 hard ceiling without a repeatable new severe cluster; the unsafe multi-target cell remains explicitly unreproducible.
    - The supplementary three-map soak exposed a separate repeatable synchronous Med-Kit completion hitch. The matched candidate-7 Med-Kit cell records every activation and exact heal, removes the completion-aligned severe frames and perceived hitches, and passes the hard median/p99 ceiling. Isolated delayed CPU stalls occur in 2/3 Harmony-only and 2/3 enabled runs without a cluster or perceived hitch, so they remain unresolved instead of prompting an evidence-free rewrite.
    ...
    - Immutable product commit `c2276aa4fc6629c4e71bb0302401ff1abc267792` passes the complete 505-test/native/package pipeline and produces the exact five-file v0.8.1 package. The 221,742-byte ZIP is SHA-256 `853f26cec49048e68554e6a2e7a3804b6aa1b9dfbbe5a91072821bcb5433563a`; its 102-byte checksum sidecar, independent extraction, transactional deployment, exact deployed-byte readback, cold activation, 33-supported/four-deliberately-unavailable capability set, and residue-free byte-identical shutdown all pass. Draft PR #9 is open and remains draft and unmerged for independent review.
