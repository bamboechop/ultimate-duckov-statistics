...
- M18 release hardening merged through [PR #19](https://github.com/bamboechop/ultimate-duckov-statistics/pull/19) on 2026-09-10 as `6e2d8b9bb7fda5b8cc0b9296cb399256315b4319`. Implementation, two independent reviews and native qualification are complete within the [accepted bounds](docs/M18_ACCEPTANCE.md). The next release is `v1.0.0-rc.1`; publication and Workshop channel verification remain separate steps. Behavior-changing RC corrections increment the RC suffix rather than becoming additional `v0.x` feature releases.
...
Post-M18 batch four is documented in [the opacity/base-distance record](docs/RC2_OPACITY_BASE_DISTANCE.md): the shared dimmer is increased to an initial 85% target for user visual acceptance. This replaces the cancelled blur investigation. Base movement is investigated with a concrete implementation proposal; tracking and persistence changes require a subsequent implementation batch.
