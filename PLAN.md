...
- Post-M10 capabilities each receive their own minor pre-release. M11 combat ownership semantics, M12 world-time, sleep, and crafted-item statistics, and M13 the native UI overhaul remain planned for `v0.11.0`, `v0.12.0`, and `v0.13.0` respectively.
- M14 is feature-frozen release hardening and the complete v1.0 audit. Its first accepted candidate is published as `v1.0.0-rc.1`; behavior-changing RC corrections increment the RC suffix rather than becoming additional `v0.x` feature releases.
...
13. **M11 — Combat ownership and observed-death semantics (`v0.11.0`, development candidate)**
...
14. **M12 — World-time, sleep, and crafted-item statistics (`v0.12.0`, planned)**
    ...
    - Add schema-12 migration, crash-safe persistence, UI, JSON, CSV, and independent capability diagnostics for world-time, sleep, and crafting. Pre-M12 history remains explicitly unavailable rather than reconstructed from current day counters, inventory state, or unlocked recipes.
    - Time/sleep acceptance covers ordinary midnight crossing, sleep crossing midnight, multi-day advancement, cancellation/interruption, restart, save-slot rotation, generation replacement, clock wrap/backward movement, and duplicate lifecycle callbacks. Crafting acceptance covers single and batched outputs, multiple-output recipes where supported, cancellation/failure, delayed collection, queue persistence across restart, unknown/modded outputs, duplicate native callbacks, and save-generation isolation. Profile/UI/export projections must agree.
15. **M13 — Native UI overhaul (`v0.13.0`, planned)**
    - Redesign the complete panel against the stable M0-M12 data model. Match Duckov's discoverable font family, sizes, colors, frames, buttons, spacing, scroll behavior, focus/navigation, and interaction feedback without redistributing proprietary game assets.
...
16. **M14 — Release hardening and v1.0 RC audit (`v1.0.0-rc.1`, planned)**
    - Freeze feature scope after M13. M14 owns compatibility, recovery, performance, packaging, documentation, installation/upgrade, accessibility/usability polish, and release defects only; new statistics or redesign work moves beyond v1.0.
    - Establish the final v1 persisted format as the only shipping baseline. Inventory and remove every schema-by-schema `0.x` migrator, legacy compatibility branch, compatibility-only serialized member, fixture, exact-migration test, and documentation path whose sole purpose is accepting unsupported M0-M13 data. Do not leave dormant readers or speculative migration infrastructure in the v1 binary for versions that no supported user could have installed.
    ...
    - Run fresh independent reviews across M0-M13 rather than treating prior milestone re-reviews as certification. A reported P1/P2 must include a feasible Duckov production path from native event or user action to observable impact; purely theoretical state-machine interleavings are retained as hardening notes unless runtime reachability is demonstrated.
...
The post-M9 order is intentional: first remove the remaining data-loss boundary, then correct combat meaning, then add the final schema-bearing world-time, sleep, and crafting families, and only then redesign every presentation surface. This leaves M14 with a feature-frozen data model and avoids both a catch-all M10 and planned `v0.9.x` feature releases.
