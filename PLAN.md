...
10. **M8.1 — Performance investigation and hot-path hardening (`v0.8.1`, active)**
    ...
    - The supplementary three-map soak exposed a separate repeatable synchronous Med-Kit completion hitch. The matched candidate-7 Med-Kit cell records every activation and exact heal, removes the completion-aligned severe frames and perceived hitches, and passes the hard median/p99 ceiling. Isolated delayed CPU stalls occur in 2/3 Harmony-only and 2/3 enabled runs without a cluster or perceived hitch, so they remain unresolved pending a representative post-hardening three-map soak instead of prompting an evidence-free rewrite.
