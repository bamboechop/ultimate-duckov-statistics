...
14. **M12 — World-time and sleep statistics (`v0.12.0`, candidate)**
    ...
    - The candidate uses public `GameClock.OnGameClockStep` plus checked `Day * 86,300 seconds + TimeOfDay` coordinates. The first observation per generation is baseline-only. New-game rotation re-baselines from the already-loaded clock between `OnNewGameReport` and `OnNewBoot`, while slot/deletion transitions await their native load observation; this preserves the new generation's automatic 07:00 boot advance without importing another slot's clock. Sleep requires the exact internal `GameClock.Step(float)` prefix/postfix result followed once by public `SleepView.OnAfterSleep`; the installed assembly has no sleep-cancel event and no other caller of that exact step method. The high-frequency clock path coalesces checked aggregate mutations into the in-memory profile once per monotonic second, schedules the full durable snapshot no more than once per 30 monotonic seconds, requests immediate durability for completed sleep, and flushes before save/profile transitions and shutdown. See [docs/M12_NATIVE_CONTRACTS.md](docs/M12_NATIVE_CONTRACTS.md).
