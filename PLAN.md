...
- [x] **Recurring freezes after returning to base — ordinary-build performance accepted by the user on 2026-09-13.** Incremental transactions now update changed records without rewriting retained runs; completed-run details load on demand. In the latest 74.077-second diagnostic base/vendor/crafting session, average UDS save-callback time fell from 23.5698 to 10.3943 ms and save-identity time from 14.4355 to 2.8955 ms relative to the previous SQLite session. The user reported noticeably smoother interaction. Both databases passed integrity/content checks; all three runs were unchanged and crafting totals advanced. The maximum recorded Update gap was 250.2338 ms; Duckov indexed-backup work explains part of one gap, while others remain unassigned. These separate sessions do not establish UDS-disabled equivalence. After deployment of the ordinary build, the user reported "Feels good!" The installed package matches the reviewed build, and the log confirms the existing generation opened and closed cleanly. A subsequent single-database ordinary-build export/restart check passed on 2026-09-13; see [single-database acceptance](docs/SINGLE_DATABASE_STORAGE.md). See [identity qualification](docs/SAVE_IDENTITY_PERFORMANCE.md); local capture evidence is retained under `artifacts/performance-identity-implementation/captures/20260913-124508`.
- [x] **Storage efficiency without information loss — completed 2026-09-13.** The initial audit is retained under `artifacts/storage-audit/20260912`. Accepted incremental SQLite persistence updates changed records without rewriting completed history, and the automatic recovery database has been removed. [Lossless record compression](docs/COMPRESSED_SQLITE_STORAGE.md) preserves precision and recorded evidence; after the native acceptance workload, the three-run primary fell from 2,387,968 to 937,984 bytes (60.7%). Copied-profile qualification also covers a 1,000-run fixture, checkpoints, export equivalence and save/reopen correctness. These are primary-file savings; WAL and other retained files are additional. The [dedicated-column comparison](docs/SQLITE_COLUMN_COMPARISON.md) measured storage, reconstruction, export CPU/allocations and changed-record writes; the user rejected blanket conversion because it did not improve on compressed records. [JSON-only ZIP export](docs/JSON_EXPORT.md) replaces the former CSV/JSON bundle, and [confirmed JSON/ZIP restore](docs/JSON_RESTORE.md) provides user-controlled restoration without maintaining a second live database. Performance and ZIP export passed native acceptance; restore received a fresh-export smoke test, with replacement of different target values covered by automated tests. No precision reduction or history truncation was introduced. Retiring user-kept older imports, exports or diagnostic artifacts is a separate cleanup decision, not unfinished implementation.
- [x] **Dedicated-column storage comparison — completed 2026-09-13; blanket conversion rejected by the user.** The disposable [comparison](docs/SQLITE_COLUMN_COMPARISON.md) measured the accepted three-run profile and existing 1,000-run fixture, including typed child tables, shared string IDs, indexes, WAL, reconstruction, export CPU/allocations and small changed-record transactions. Compacted compressed primaries are 925,696 and 66,662,400 bytes; fully converted shared-string columns are 1,019,904 and 79,060,992 bytes (+10.18% and +18.60%). Keeping completed runs compressed and converting only mutable data saves 4,096 bytes on the recorded profile but adds 77,824 bytes on the larger fixture. All original record values and all 37 export streams agree across representations; precision, missing/null/empty states, ordering and rollback/reopen checks pass. Timings include a JSON reconstruction bridge and are not a direct typed-loader or native frame-time comparison. The user decided to drop the blanket conversion to dedicated columns and retain the current compressed SQLite storage. No production storage replacement or live conversion was made. This closes the conversion proposal; it does not prohibit typed columns for a separately justified feature.
- [x] **Single SQLite database — implemented, deployed and user-accepted 2026-09-13.** Removed the continuously updated recovery database after the accepted two-copy build. Retain FULL-synchronized WAL transaction durability, failed-write retry ownership, save-generation isolation, recorded-value accuracy, export snapshots and reset safety. The user accepts possible loss of statistics if the single database becomes corrupt. Existing primary databases open directly with the same schema; obsolete recovery files are not promoted or maintained, and their local retirement follows primary validation. See [the single-database policy](docs/SINGLE_DATABASE_STORAGE.md). User-initiated restore/import is implemented separately through [Diagnostics](docs/JSON_RESTORE.md), with content validation, same-slot selection and confirmed generation replacement. The user smoke-tested a fresh-export restore and approved committing it; older-value replacement is covered by automated tests.
- [x] **English map naming in Runs and Records — user accepted 2026-09-12.** The shared map-name resolver now attempts translation even when the display-name key equals the scene ID, retaining the recorded name when no usable native translation exists. Automated checks cover matching and differing IDs/keys, missing or blank translations, cached lookups, language changes, reopening, and unchanged profile data. After deployment, the user confirmed `J-Lab Facility` in English and `J-Lab-Anlage` in German.
- [x] **Partial values and event attribution — new-run UI accepted 2026-09-12; export/restart persistence verified 2026-09-13.** Runs uses compact partial markers and independent run totals. The underlying correction preserves same-map origins when buff reapplication changes equipment, observes non-player effect origins without borrowing player equipment, and separates source/outcome association completeness from per-map outcome totals through capture, persistence, recovery, aggregation, UI, and exports. Genuine destination gaps remain partial; bounded first-failure diagnostics retain event type, time, and endpoint identities. Existing historical flags remain conservative. See [run attribution](docs/RUN_ATTRIBUTION.md). Deployed on 2026-09-12. The user confirmed that the new five-segment, four-map run ending in death shows no unwanted partial values in the summary or route. The 2026-09-13 single-database acceptance export and reopened database retain identical run contents: both newer runs keep complete attribution, while the original run retains its conservative historical flag. The exact event that first caused the original incident was not retained.
- [x] **Unknown weapon in Combat > Weapons & ammunition — user accepted 2026-09-12.** A consistent unknown combat bucket now shows as Unattributed combat, with its recorded outgoing damage, player kills, and recorded effect/environmental/unclassified kill counts. It explains that these values are already included in overall totals; no gun, firing count, accuracy, or ammunition pairing is inferred. Unknown firing and mismatched identities remain separate. The nonpositive weapon-name fallback no longer looks up body metadata. The precise historical effect was not retained. The user confirmed the deployed Unattributed combat presentation is fixed on 2026-09-12.
- [x] **Combat > Summary quick-glance layout — user accepted 2026-09-12.** Overall accuracy was introduced in commit `6cde1d7` with the Combat accuracy follow-up. The revised layout retains it: Kills by you and Deaths occupy equal halves above Damage dealt, Damage taken, and Overall accuracy in equal thirds. Narrow surfaces use full-width cards where needed.
- [x] **Economy > Contexts > Raid text fit — user accepted 2026-09-12.** The inflow column now measures the `of which proven acquired` value as well as its ordinary numeric cells, so `Unavailable` receives sufficient width. Narrow layouts reserve the measured value width or place it below the label.

The screenshots and read-only incident evidence are retained locally with the performance investigation; gameplay logs, profiles, and native assembly excerpts are not release artifacts.
...
- [x] **GitHub 1.1.x publication:** encounter history shipped in [v1.1.0](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v1.1.0) and kill-distance highlights shipped in [v1.1.1](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v1.1.1) on September 16, 2026. The published archives retain their original contents.
...
**Overview kill-distance highlights shipped in GitHub 1.1.1.** The [distance rules and qualification](docs/COMBAT_ENCOUNTER_HISTORY.md#overview-highlights--kill-distance-111) cover recorded player/enemy positions from record-eligible completed runs and the user-accepted September 16 UI check. Existing encounter records participate without a schema change.
...
## Optional mod compatibility and 1.3.0 delivery

- [x] **Inspected mod integrations:** First Person Camera headshots, Become Veteran item healing, independent shared buff observation and Storage Search Bar coexistence merged through [PR #31](https://github.com/bamboechop/ultimate-duckov-statistics/pull/31). The user accepted the exercised FPC-enabled/disabled gameplay and exports on September 23, 2026.
- [x] **Native F8 pause:** opening at base pauses gameplay and the inspected FPC camera; closing releases only the pause UDS owns. The user confirmed the native behavior on September 23.
- [x] **1.3.0 publication — September 23, 2026:** version preparation merged through [PR #32](https://github.com/bamboechop/ultimate-duckov-statistics/pull/32). Release source `259ee1c1485059454429eb2ea32aa7e00a198070`, reproducible binaries/ZIP, tag and public downloads passed [independent verification](docs/V1_3_0_PUBLICATION.json).
- [ ] **Healing notice wording:** Item Use must distinguish historical incomplete healing from currently unavailable capture. Recorded values and completeness evidence already retain the distinction; see the [observed limitation](docs/MOD_COMPATIBILITY.md#native-smoke-test-2026-09-23).
- [ ] **Broader optional-mod native checks:** the [remaining matrix](docs/MOD_COMPATIBILITY.md#broader-native-checks) covers unexercised split-healing, projectile, lifecycle and frame-time cases. The accepted smoke test does not establish those outcomes.
...
- No unsupported pre-v1 persisted-data upgrade path ships in v1. Current-format validation and recovery remain required.
...
- M16 adds exact item-resource consumption for proven successful crafts and lossless output-resource associations as `v0.16.0`. Implementation, independent review, and user-controlled qualification are complete; [PR #17](https://github.com/bamboechop/ultimate-duckov-statistics/pull/17) merged as `4f24b026060e6dd883881c572411ec82b7812631`, and the [v0.16.0 GitHub pre-release](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v0.16.0) was published on 2026-08-30. M17 completes the retained-native UI overhaul plus the schema-17 Runs and schema-18 Equipment foundations as `v0.17.0`; implementation, independent review and re-review, and user-controlled qualification are complete, and [PR #18](https://github.com/bamboechop/ultimate-duckov-statistics/pull/18) merged as `b3c7e882e1090b4b302990051042f7c103f48653`. The lightweight `v0.17.0` tag resolves to release commit `18f1d068912f4f54b7efc6195c248106d5d8d8fe`, and the [GitHub pre-release](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v0.17.0) was published on 2026-09-07.
- M18 release hardening merged through [PR #19](https://github.com/bamboechop/ultimate-duckov-statistics/pull/19) on 2026-09-10 as `6e2d8b9bb7fda5b8cc0b9296cb399256315b4319`. Implementation, two independent reviews and native qualification are complete within the [accepted bounds](docs/M18_ACCEPTANCE.md). RC1 and RC2 publication are recorded in [the RC2 release record](docs/RC2_RELEASE.md). GitHub `v1.0.0` subsequently included German localization/layout and About link removal. Published `v1.3.0` adds optional-mod compatibility and native base pause to the retained UI and 1.1.x features; first Workshop publication and channel verification remain separate steps.
...
- **Core:** normalized events, reducers, records, classification, current-format persistence/recovery, export, and capability models. It must not depend on Unity or Duckov.
...
`%USERPROFILE%\AppData\LocalLow\TeamSoda\Duckov\UltimateDuckovStatistics\v1\`
...
- Export one machine-readable `statistics.json` file inside `statistics.zip`, with the complete structured data. CSV export was retired after v1; historical delivery evidence below records the formats available at that time.
...
- Show a compact ordered route in Runs, with expandable per-segment details. Export the complete route and segments in JSON.
...
8. **M7 — Containers (`v0.7.0`, merged and released after complete automated and manual acceptance)**
    ...
    - Lifetime/map/run aggregation, retained Runs presentation, JSON, and `containers.csv` retain exact native evidence.
9. **M8 — Multi-map runs and route attribution (`v0.8.0`, merged after implementation, gameplay acceptance, and review hardening)**
    ...
    - Add route/segment capability reporting, crash-safe active-segment checkpoints, Runs route presentation, JSON, and flattened route/segment CSVs.
...
11. **M9 — Economy (`v0.9.0`, merged and released after complete automated and manual acceptance)**
    ...
    - Omit Cash terminal disposition on Duckov 2.3.30: fungible main/pet/storage ownership cannot prove acquired-unit secured/lost disposition. Preserve proven acquisition without terminal buckets.
    - Preserve exact-once deferred lifetime/checkpoint recovery, run/segment/starting-map/route-map aggregation, retained Economy/Overview/Runs UI, supported capability rows, JSON and currency/cash-acquisition CSVs.
...
12. **M10 — Lossless route-event association (`v0.10.0`, published pre-release)**
    ...
    - Accepted rows reduce exactly into checked 64-bit buckets keyed by event family plus source/outcome segment. Repeated visits and delayed outcomes remain distinct; cardinality is bounded by five families and 64 segments. Current partial attribution stays explicit. Economy uses independent cursor/segment fan-out.
...
14. **M12 — World-time and sleep statistics (`v0.12.0`, published)**
    ...
    - Preserve crash-safe persistence, UI, JSON, CSV and independent capabilities for calendar-day advancement, observed world time, completed sleep sessions and sleep-advanced time.
...
15. **M13 — Crafted-item statistics (`v0.13.0`, published)**
    ...
    - Preserve crash-safe persistence, UI, JSON, CSV and independent capabilities for proven crafting completion, produced quantity and recipe/batch identity.
    ...
    - The installed Duckov 2.3.30 contract has no persisted craft queue or separate collection lifecycle. The adapter correlates private `Craft(CraftingFormula)` with successful `Cost.Return` output delivery; downstream callback failure cannot hide a proven delivery. It records singular declared output quantity, numeric output ID, recipe ID and batch distribution. Overlap-safe tokens publish once; failed payment or incomplete delivery records nothing. M16 independently proves resources and total charge. Workstation/run/map context and multiple-output metrics are omitted; see [native evidence](docs/M13_NATIVE_CONTRACTS.md).
...
16. **M14 — Lossless weapon and equipment-slot associations (`v0.14.0`, complete and merged)**
    ...
    - Preserve unknown, modded, and partially available identities without guessing compatibility from caliber, weapon metadata, inventory state, or matching aggregate totals. A callback with only one proven identity continues to update that independently supported aggregate but cannot fabricate a pair.
    ...
    - Keep exact item-tree signatures, structured observed membership, loadout identity, transition history, recurring-loadout semantics and durations. Joint identities are not replaced by marginal child-item totals, and missing membership cannot be reconstructed from hashes.
...
17. **M15 — Current economy holdings (`v0.15.0`, published pre-release)**
    ...
    - Add schema 15, exact save-generation persistence and recovery where a retained last-observed snapshot is required, observation timestamps/freshness, independent Money/Cash/liquid-wealth capabilities, Diagnostics, `statistics.json`, flattened CSV output, and temporary pre-overhaul Overview/Economy UI projection.
...
18. **M16 — Crafting-resource consumption statistics (`v0.16.0`, published pre-release)**
    ...
    - Preserve event-time costs rather than recomputing history as current recipe cost multiplied by historical action count. A Duckov update or runtime formula change must not rewrite the meaning of earlier crafts.
    - Treat currency independently from item resources. If the installed audit proves every recipe has `Cost.money == 0`, record that proven scope and omit currency from the normal resource ranking. If any recipe charges currency, discover and capability-gate the exact native evidence required to distinguish the total charge and any Money/Cash split; never relabel a declared charge, M9 flow, or inferred balance difference as exact crafting spend without that proof. The payment hook is a cost-only dependency: a cold conflict or later drift disables item-resource and currency evidence, invalidates both dimensions for in-flight scopes, and leaves M13 completion, quantity, output, recipe, and batch tracking active through the independently trusted craft/delivery pair.
...
19. **M17 — Native UI overhaul (`v0.17.0`, complete and merged)**
    - The [schema-18 Equipment foundation](docs/M17_EQUIPMENT_DATA_FOUNDATION.md) adds bounded exact loadout/active-set definitions and typed direct/tote duration evidence with prospective historical-unavailable handling. The [retained Equipment tab](docs/M17_RETAINED_EQUIPMENT.md) consumes it across Loadouts, Weapons, Armor & gear and Totems, with exact Runs navigation, independent scrolling and measured narrow layouts. Visual, audio, and gameplay acceptance completed through the user-controlled manual gate.
    - The Equipment Most-used loadout card ranks every exact lifetime loadout by total active duration, then ordinal loadout ID, including identities observed in one completed run. Show the actual occurrence count and truthful composition availability. This UI selection is independent of the explicitly recurring `recurring_loadouts.csv` export, which retains its two-run filter; exact identity, duration/occurrence tracking and per-run summaries retain their existing meanings.
    - The [retained Combat tab](docs/M17_RETAINED_COMBAT.md) presents Summary, Enemies, Weapons & ammunition and Incoming damage from exact-generation Combat/Weapon projections, with truthful ownership and pairing limitations, bounded native scrolling and responsive focus. Native visual and audio qualification completed under the user-controlled M17 protocol.
    - The [retained Records tab](docs/M17_RETAINED_RECORDS.md) presents authoritative extraction/death duration references and complete starting-map cards in one bounded document, with exact Runs navigation and shared native scrolling. Screenshot, audio, focus, and live lifecycle qualification completed under the user-controlled M17 protocol.
    - [Economy](docs/M17_RETAINED_ECONOMY.md), [Crafting](docs/M17_RETAINED_CRAFTING.md), [Item Use](docs/M17_RETAINED_ITEM_USE.md), and [Diagnostics](docs/M17_RETAINED_DIAGNOSTICS.md) complete the retained tab composition. They consume existing exact recorded evidence, preserve independently unavailable/stale values, and expose bounded native expansions, filters and scrolling. Diagnostics integrates configurable access, actual capability health and single-flight export/reset with safe archive/new-generation results. All nine tabs are mounted, and their in-game screenshot, audio, and interaction acceptance completed under the user-controlled M17 protocol.
    - Add the [schema-17 Runs data foundation](docs/M17_RUNS_DATA_FOUNDATION.md): fresh terminal equipment captured at `OnRaidDead` / `OnEvacuated`, deferred death completion, and same-event attack-kind player-kill partitions with explicit unknown attribution. The foundation is consumed by the [complete retained Runs tab](docs/M17_RETAINED_RUNS.md), including exact Overview navigation, terminal equipment, run combat, responsive layout and bounded scrolling; native visual and interaction qualification completed under the user-controlled M17 protocol.
...
20. **M18 — Release hardening and v1.0 RC audit (`v1.0.0-rc.1`)**
    - Implementation merged as `6e2d8b9bb7fda5b8cc0b9296cb399256315b4319` after two independent reviews. Native qualification and its accepted performance/coverage limits are recorded in [M18 acceptance](docs/M18_ACCEPTANCE.md); final release packaging, publication and Workshop channel verification remain separate steps.
    ...
    - Audit every shipped UI construction, open, tick, localization, and responsive-layout path for acceptance-only `pixel perfect` validation. Remove production checks and advisory wrappers that compare live controls with mock/reference coordinates, exact dimensions, expected-language widths, typography, materials, styling, hierarchy, fit, overflow, or other aesthetic expectations. No finite runtime measurement, longer localized label, content overflow, font-rendering difference, Unity-normalized presentation property, or mismatch from a reference image may suppress or destroy the panel, a control, or either access path; a visually imperfect but operable UI must remain visible and use its defined wrapping, clipping, scrolling, or overflow behavior. Exact visual, dimension, and fit assertions belong only in deterministic tests and user-controlled screenshot validation. Retain runtime rejection only for narrowly defined states in which the UI cannot be constructed or operated safely, such as a missing required canvas, asset, or component; a non-finite or otherwise unusable measurement; an unexpected construction failure; or broken resource/listener lifecycle ownership. Add source/package audit evidence that no production visual-acceptance or overflow-triggered shell-suppression path remains, plus regressions proving long localized labels and overflowing content cannot disable Statistics-button or F8 access or tear down an otherwise operable shell.
...
- [x] **About:** Add the tenth tab before Diagnostics, with the mod description, author and translation invitation. The original support/Discord buttons were removed for Workshop preparation; volunteers are directed to Steam Workshop comments.
...
- [x] **German localization and layout:** English/German live switching, retained caption refresh, Overview wrapping/column spacing, tab clipping and clickable overflow controls; see [the implementation record](docs/GERMAN_UI_LAYOUT.md).

The [RC2 pre-release](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v1.0.0-rc.2) was published and independently verified; [the publication receipt](docs/RC2_PUBLICATION.json) records its exact tag and asset identities. Source/build metadata and current documentation identify published `1.3.0`; the historical GitHub `v1.0.0` release remains unchanged. Before first public distribution, the user chose schema 1 as the fresh baseline instead of preserving development schema-18 statistics. The format ID remains `uds-profile-v1` and directory `v1`; no development-data migration or automatic deletion code ships. Final source qualification, publication and the first Workshop listing/subscription verification follow [docs/RELEASE_PROCESS.md](docs/RELEASE_PROCESS.md). Live publication and CI state belong on GitHub.
...
- Current-format serialization round-trips, semantic validation, corruption recovery, and backup/temporary fallback work.
