...
11. **M9 — Economy (`v0.9.0`, immutable candidate accepted; draft PR open)**
    ...
    - Current candidate evidence: the full live economy matrix proves exact Money/Cash deltas, semantic unknown retention, failed-operation exclusion, split/merge neutrality, hydration exclusion, export agreement, deferred base/shop durability, and residue-free shutdown. Scalable activation/sequence idempotency then passed 100,000-event and recovery stress, and the sequence-zero correction at `0b223e45bbafba8cbbe7f5d2213cd09588b5b870` passed deployment plus a minimal cold persistence check. A subsequent review invalidated that artifact as the final candidate: production `AddAndMerge` could consume a dropped Cash item before reconciliation and fabricate a new acquisition, level initialization could discard a dirty exact Cash delta, and checkpoint-only arithmetic saturation could be lost during lifetime recovery. Correction commit `20fa8acdc873e331515d9155ed838b96cb820b7a` retains the exact last-owned amount for consumed re-pickups, drains Cash before hydration suspension, and treats saturation-only checkpoint state as a recoverable delta. The three production-order regressions and complete Debug/Release suites pass 609/609; the installed-game contract probe, frame-time analyzer, formatter, source-safety gate, warning-free Core/native builds, package verifier, and independent extraction pass. The independently audited 253,758-byte replacement ZIP is SHA-256 `6157fa88e242686f10b9c6a3db63de2b10c7d51cbec807cb2d1f74fbc221cb15`. After fresh exact-hash approval it deployed transactionally with unchanged save/profile trees during promotion. Cold activation preserved a new sequence-zero cursor; a focused production `AddAndMerge` raid then recorded exact Cash `+34/-34`, `UnknownAdjustment`, Raid, with acquired/unresolved both zero in lifetime, run, segment, JSON, and CSV. Normal shutdown produced byte-identical profiles, clean diagnostics/logs, and no runtime or deployment residue. [Draft PR #10](https://github.com/bamboechop/ultimate-duckov-statistics/pull/10) remains open and unmerged.
