...
16. **M14 — Lossless weapon associations (`v0.14.0`, planned)**
    ...
    - At the existing public item-tree observation boundary, retain structured attachment-slot state alongside each unchanged exact-set signature using stable weapon `Item.TypeID`, native attachment slot key/path, and either an occupied attachment `Item.TypeID` plus non-canonical display metadata or a proven `Empty` state. A missing slot, unreadable tree, or degraded capability is `Unavailable`, never `Empty`. Track the time that the weapon is slotted while each named attachment is installed or each existing attachment slot is proven empty; this is equipped duration, not actively held/selected-weapon duration. Use compact checked aggregates or an equivalently lossless set-membership projection rather than an unbounded raw interval journal, and preserve the existing monotonic active-raid clock, pause/loading exclusion, event-driven invalidation, one-second reconciliation, and deferred persistence behavior.
    - Expose a weapon-first equipment projection listing total equipped time as the checked sum of that weapon identity's character-slot durations, the separate duration in each weapon slot, and attachment-slot duration grouped by Scope, Muzzle, Grip, Stock, Tactics, and Magazine. Each group lists named attachments plus a localized `No scope`, `No grip`, or equivalent row when that exact native slot was proven present and empty. Native slot keys remain authoritative; preserve unknown or modded slot paths and attachment identities instead of forcing them into a built-in category. Never infer an empty interval from an absent slot, missing attachment metadata, or the difference between weapon and named-attachment totals.
    - Publish the two association families' independent capability and historical-availability states consistently through profile, current UI model, Diagnostics, `statistics.json`, and dedicated CSV output. In a correlation-supported scope, summed weapon-ammunition pair counts by weapon and by ammunition must reconcile with the corresponding independently captured totals after excluding only events explicitly marked uncorrelated because one identity was unavailable. For each weapon and one attachment slot/path, named-attachment and proven-empty durations are mutually exclusive and cannot exceed that weapon's equipped duration in the same scope; equality is required only for intervals in which that slot's existence and state were fully observable, and any remainder stays unavailable. Different attachment categories are simultaneous dimensions and are not summed together as a total.
    ...
    - Deterministic weapon-attachment coverage includes occupied-to-empty-to-occupied changes, a weapon without a particular slot versus one with that slot proven empty, attachment replacement/removal, the same named attachment on multiple weapons, the same weapon identity in different character slots, nested and modded slot paths, display-name enrichment, pause/loading exclusion, repeated-map routes, checkpoint/restart/interruption recovery, save-slot/new-game/reset transitions, capability degradation, arithmetic limits, export/UI agreement, and shutdown cleanup. Manual acceptance changes at least one safely available attachment during a run, retains a proven-empty interval, records approximate active intervals without treating wall-clock observation as exact native evidence, and confirms the resulting weapon, character-slot, exact-set, named-attachment, and empty-slot projections across UI, JSON, CSV, persisted profile, restart, and clean shutdown.
