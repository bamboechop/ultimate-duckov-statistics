...
- M14 preserves lossless weapon and equipment-slot associations as `v0.14.0`: accepted firing actions by weapon-ammunition pair plus equipped duration for proven character-slot state and nested equipped-item slot membership. M15 then applies the native UI overhaul to the stable M0-M14 data model as `v0.15.0`.
