...
- `v0.12.0` is the published M12 world-time and sleep statistics pre-release. The M13 crafted-item-statistics implementation is the `v0.13.0` draft candidate; M14 the native UI overhaul remains planned for `v0.14.0`.
...
15. **M13 — Crafted-item statistics (`v0.13.0`, draft candidate)**
    ...
    - Schema 13 uses checked lifetime/output/recipe/batch aggregates and the existing deferred atomic profile writer, with explicit export/profile/save-transition/shutdown barriers and no fixed event-history ceiling. `statistics.json`, `crafting_totals.csv`, `crafting_recipes.csv`, Overview, Crafting, and Diagnostics expose the same current capability and historical-availability semantics. The authoritative mutable delivery surface is the [main-to-feature comparison](https://github.com/bamboechop/ultimate-duckov-statistics/compare/main...feat/crafted-item-statistics).
