...
- Post-M9 capabilities each receive their own minor pre-release. M10 lossless route association has completed local candidate qualification on `feat/lossless-route-association` and remains unreleased on draft PR #11; M11 combat ownership semantics, M12 world-time and sleep statistics, and M13 the native UI overhaul remain planned for `v0.11.0`, `v0.12.0`, and `v0.13.0` respectively.
- M14 is feature-frozen release hardening and the complete v1.0 audit. Its first accepted candidate is published as `v1.0.0-rc.1`; behavior-changing RC corrections increment the RC suffix rather than becoming `v0.9.x` feature releases.
...
14. **M12 — In-game days and sleeping time (`v0.12.0`, planned)**
    ...
    - Add schema-12 migration, crash-safe persistence, UI, JSON, CSV, and capability diagnostics. Pre-M12 history remains explicitly unavailable rather than reconstructed from current day counters.
    - Acceptance covers ordinary midnight crossing, sleep crossing midnight, multi-day advancement, cancellation/interruption, restart, save-slot rotation, generation replacement, clock wrap/backward movement, and duplicate lifecycle callbacks with profile/UI/export agreement.
...
The post-M9 order is intentional: first remove the remaining data-loss boundary, then correct combat meaning, then add the final schema-bearing statistic family, and only then redesign every presentation surface. This leaves M14 with a feature-frozen data model and avoids both a catch-all M10 and planned `v0.9.x` feature releases.
