...
19. **M17 — Native UI overhaul (`v0.17.0`, implementation candidate)**
    - The [schema-18 Equipment foundation](docs/M17_EQUIPMENT_DATA_FOUNDATION.md) adds bounded exact loadout/active-set definitions and typed direct/tote duration evidence with prospective historical-unavailable handling. The [retained Equipment tab](docs/M17_RETAINED_EQUIPMENT.md) consumes it across Loadouts, Weapons, Armor & gear and Totems, with exact Runs navigation, independent scrolling and measured narrow layouts. Visual/audio/gameplay acceptance remains the user-controlled manual gate.
