...
- [ ] **Storage efficiency without information loss — initial read-only audit complete 2026-09-12.** The copied three-run profile is 1,925,716 bytes; fast gzip produces 255,681 bytes with byte-exact recovery. The observed primary/backup/support/export footprint is 7,050,243 bytes across 43 files. Equipment/combat scope repetition and verbose metadata dominate; the 37-file export can be archived losslessly to 259,487 bytes with ZIP level 6. Local audit evidence is retained under `artifacts/storage-audit/20260912`. Incremental SQLite persistence is implemented and gameplay-tested: routine saves update changed records, retaining normally immutable completed runs and explicit corrections. JSON generation remains an export operation. Automatic recovery-copy persistence is removed and accepted. The [lossless record-compression implementation](docs/COMPRESSED_SQLITE_STORAGE.md) now converts the actual SQLite format-6 predecessor to format 7 once, preserving public schema 1 and all original record bytes. Copied-profile qualification on 2026-09-13 reduced primary size from 2,387,968 to 962,560 bytes (three runs), and from 482,627,584 to 64,114,688 bytes (1,000 runs). Full-profile/checkpoint hashes, logical keys/checksums and all 37 real-profile export files agree with the preceding implementation after conversion and save/reopen. Debug and Release each pass 2,204 main tests and 90 shell tests. Ordinary-build compression acceptance passed on 2026-09-13: both 37-file exports succeeded across a clean restart, all three completed runs remained byte-exact, and new crafting, sales and base movement persisted. The user reported smooth base interaction with only tiny occasional spikes. The actual primary fell from 2,387,968 to 937,984 bytes (60.7%); all 1,630 record checksums and copied-database integrity passed. See the compression document for the bounded gameplay evidence and retained WAL footprint; primary compression does not imply equivalent WAL or whole-directory savings. ZIP-export implementation and any retirement of older import/export files remain separate follow-ups. Compare complete read/write time, CPU, allocations, and recovery behavior on representative and larger valid profiles, including active-run checkpoints. Do not reduce precision, discard recorded detail, shorten history, or weaken data safety to meet the size goal. Avoid trading smaller files for worse gameplay performance or unnecessary runtime complexity.
- [ ] **Dedicated-column storage comparison — next investigation, user decision 2026-09-13.** Compare the accepted compressed JSON-record format with dedicated SQLite columns and relational child tables for the remaining payloads. Measure the actual three-run profile and the existing 1,000-run fixture, including indexes, WAL, startup and run-detail reads, changed-record writes, export time, CPU and allocations. Preserve every value, numeric precision, ordering, identity and unknown/partial state, incremental saves and transaction durability. Determine whether the storage and runtime benefit justifies a conversion and added schema complexity before implementing or deploying a replacement; smaller storage is an open question, not an assumed result.
- [x] **Single SQLite database — implemented, deployed and user-accepted 2026-09-13.** Removed the continuously updated recovery database after the accepted two-copy build. Retain FULL-synchronized WAL transaction durability, failed-write retry ownership, save-generation isolation, recorded-value accuracy, export snapshots and reset safety. The user accepts possible loss of statistics if the single database becomes corrupt. Existing primary databases open directly with the same schema; obsolete recovery files are not promoted or maintained, and their local retirement follows primary validation. See [the single-database policy](docs/SINGLE_DATABASE_STORAGE.md). User-initiated restore/import from exported JSON remains a separate feature requiring content validation and destination-generation rules.
...
- Export one machine-readable `statistics.json` file with the complete structured data. CSV export was retired after v1; historical delivery evidence below records the formats available at that time.
