...
This guide accompanies **Ultimate Duckov Statistics 1.1.0**, including Map & Kills, SQLite persistence improvements and JSON/ZIP export and restore. English and German are included. Version 1.1.0 is being prepared for the first Steam Workshop listing and subscription-install verification; consult [GitHub releases](https://github.com/bamboechop/ultimate-duckov-statistics/releases) for published packages. The supported Workshop upgrade baseline begins when 1.1.0 is explicitly distributed through that verified channel. The earlier [GitHub 1.0.0 archive](https://github.com/bamboechop/ultimate-duckov-statistics/releases/tag/v1.0.0) retains its original JSON-only runtime. Earlier `0.x` builds and v1 release candidates were voluntary testing artifacts.
...
Runs → **Map & Kills** records encounters automatically. Select a map visit to reveal its walking path and dotted teleport connections, or select an encounter from the whole-run feed to focus its map. Repeated visits remain separate. Details include recorded damage, weapons, ammunition, hit/headshot evidence and inspected enemy inventory with kept-item highlighting. Missing map artwork does not hide the encounter list. Earlier aggregate-only runs cannot be backfilled, and inventory that was never inspected remains unknown.
...
UDS stores current statistics in one `profile.sqlite` database. A save is acknowledged after its changed records commit in a durable SQLite transaction using WAL and FULL synchronization. There is no automatic recovery database; corruption of the primary can make the statistics unavailable. Larger records, including completed-run detail, use lossless compression inside SQLite; small records stay raw. Routine changes update affected records without rewriting retained history. Keep the primary together with any `-wal`, `-shm`, ownership and read-failure evidence files when backing up or investigating a problem. Close Duckov before manually copying its data directory. Previous SQLite builds may have left `.recovery` files; this build neither uses nor updates them. Retiring those files requires a separate check of the valid primary, as described in [the single-database policy](https://github.com/bamboechop/ultimate-duckov-statistics/blob/main/docs/SINGLE_DATABASE_STORAGE.md).

When no SQLite store exists, a valid current-format JSON profile is imported through a fully validated staging database. The original JSON and its recovery candidates remain intact. Once SQLite exists it is authoritative; failed SQLite recovery does not silently revert to the old JSON. Do not downgrade to a JSON-only build against statistics changed in SQLite: older JSON-only builds cannot read those new changes. Retain the whole pre-test data-directory backup for an intentional rollback. SQLite's internal record schema is independent of the unchanged public JSON export format. The [compression update](https://github.com/bamboechop/ultimate-duckov-statistics/blob/main/docs/COMPRESSED_SQLITE_STORAGE.md) converts internal format 6 to 7 on first opening, followed by one-time space reclamation. This first open may take longer and needs temporary disk space. Older SQLite builds cannot read the converted database; reverting only the mod files does not undo conversion.

Diagnostics exports an immutable snapshot of the captured generation as `statistics.zip`, containing a single `statistics.json`, under the v1 data root. CSV export has been retired. Paths remain available when clipboard copying fails. Treat exports as local gameplay data and inspect them before sharing. Diagnostics can also restore a saved JSON or ZIP export for the same save slot after validation, preview and explicit confirmation; see [the restore procedure](https://github.com/bamboechop/ultimate-duckov-statistics/blob/main/docs/JSON_RESTORE.md).
