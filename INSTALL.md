...
- Each lifetime, run, segment, starting-map, or route-map economy aggregate retains at most 2,048 exact event IDs. At saturation it preserves all prior totals and identities, exposes `deduplication_saturated` in JSON/CSV/UI diagnostics, and disables further capture for that aggregate rather than evicting evidence and risking replay double counts. Terminal resolution of already-recorded Cash acquisition remains separate.
