...
In this build, opening UDS with F8 at base also opens Duckov's native pause menu, pausing gameplay while statistics are visible. Closing UDS resumes gameplay only when UDS initiated that pause. If the game was already paused, closing UDS returns to the pause menu. The native pause state also blocks camera movement in the inspected First Person Camera build. Main-menu access does not start a base pause. If the native base pause menu is unavailable, F8 cannot open an unpaused statistics overlay there.
