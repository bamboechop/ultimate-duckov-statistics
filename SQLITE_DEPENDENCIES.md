...
Database connections use WAL with FULL synchronization. Related changed records, generation metadata and bounded receipts commit together. Success also requires the independent recovery database to commit. Passive WAL maintenance runs on separate workers using loading/sleep hints and a long-session fallback. This preserves sync guarantees; it is not a NORMAL/OFF durability tradeoff. Current domain records remain a hybrid of typed columns and JSON BLOBs, and both databases add storage overhead. JSON and CSV exports remain generated on demand.
