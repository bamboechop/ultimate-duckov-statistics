...
Database connections use WAL with FULL synchronization. Related changed records, generation metadata and bounded receipts commit together. The primary commit is the durability acknowledgement; there is no independent recovery database. Passive WAL maintenance runs on a separate worker using loading/sleep hints and a long-session fallback. This preserves sync guarantees; it is not a NORMAL/OFF durability tradeoff. Current domain records remain a hybrid of typed columns and JSON BLOBs; further representation and storage-size optimization is separate work. Existing primary databases keep the same storage schema. Old recovery files are ignored rather than automatically deleted or promoted. JSON and CSV exports remain generated on demand.
